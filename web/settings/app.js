(() => {
  ...
  const FIELD_META = {
    ...
    "llm.backend": {
      label: "Backend",
      description: "Use LM Studio, a generic external server, or a Project Akira-managed llama.cpp process.",
      options: [
        { value: "lm_studio", label: "LM Studio" },
        { value: "llama_cpp", label: "Managed llama.cpp" },
        { value: "openai_compatible", label: "OpenAI-compatible server" },
      ],
      advanced: true,
    },
    "llm.base_url": { label: "External server URL", description: "Used by LM Studio and generic OpenAI-compatible backends. Managed llama.cpp chooses its own local URL.", fullWidth: true },
    "llm.api_key": { label: "API key", inputType: "password", advanced: true },
    "llm.model": { label: "External model name", description: "Used by LM Studio or another external server. Managed llama.cpp uses its configured model alias.", fullWidth: true },
    "llm.llama_cpp_executable": { label: "llama-server executable", description: "Full path to llama-server.exe. Leave empty to use a bundled copy or llama-server on PATH.", fullWidth: true },
    "llm.llama_cpp_model_path": { label: "llama.cpp GGUF model", description: "Full path to a local .gguf model. The download manager is added in issue #33.", fullWidth: true },
    "llm.llama_cpp_model_alias": { label: "llama.cpp model alias", description: "Stable model name sent to the local OpenAI-compatible API." },
    "llm.llama_cpp_host": { label: "llama.cpp host", description: "Managed servers are restricted to localhost.", advanced: true },
    "llm.llama_cpp_port": { label: "llama.cpp port", min: 1, max: 65535, step: 1 },
    "llm.llama_cpp_context_size": { label: "llama.cpp context size", min: 256, max: 1048576, step: 256 },
    "llm.llama_cpp_gpu_layers": { label: "GPU layers", description: "Use auto, all, or a non-negative layer count. Hardware presets arrive in issue #34." },
    "llm.llama_cpp_threads": { label: "CPU threads", description: "-1 lets llama.cpp choose automatically.", min: -1, max: 512, step: 1 },
    "llm.llama_cpp_startup_timeout_seconds": { label: "Startup timeout", description: "Seconds to wait for the model to load before reporting an error.", min: 1, max: 600, step: 1 },
    "llm.llama_cpp_extra_args": { label: "Extra llama.cpp arguments", description: "One complete command-line argument per line. Advanced use only.", fullWidth: true, advanced: true },
    ...
    "llm.reasoning_mode": { label: "Reasoning mode", description: "LM Studio supports all listed levels. Managed llama.cpp currently supports off, auto, or on.", options: ["off", "auto", "low", "medium", "high", "on"] },
